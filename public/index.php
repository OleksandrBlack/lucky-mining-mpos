...
*/
header("Access-Control-Allow-Origin: https://lucky-mining.com.ua");
  ...
  if (!$skip_check) {
    ...
    if ($mcad->rate_limit_site_request) {
      if (!is_array($error_page) || count($error_page) < 1 || (empty($error_page['page']) && empty($error_page['action']))) {
        die("Вы слишком быстро отправляете слишком много запросов!");
      } else {
...
if (count(@$_SESSION['last_ip_pop']) == 2) {
  ...
  if (@$_SESSION['AUTHENTICATED'] && $_SESSION['last_ip_pop'][0] !== $user->getCurrentIP()) {
    $_SESSION['POPUP'][] = array('CONTENT' => "Последний раз вы заходили с адреса <b>$ip</b> в $time", 'DISMISS' => 'yes', 'ID' => 'lastlogin', 'TYPE' => 'alert alert-warning');
  } else {
    $_SESSION['POPUP'][] = array('CONTENT' => "Последний раз вы заходили с адреса <b>$ip</b> в $time", 'DISMISS' => 'yes', 'ID' => 'lastlogin', 'TYPE' => 'alert alert-info');
  }
}
...
if (!empty($action)) {
  $debug->append('Загрузка действий: ' . $action . ' -> ' . $arrActions[$action], 1);
  require_once(PAGES_DIR . '/' . $page . '/' . $arrActions[$action]);
} else {
  $debug->append('Загрузка страницы: ' . $page . ' -> ' . $arrPages[$page], 1);
...
$debug->append("Загрузка отладочной информации в шаблон", 4);
