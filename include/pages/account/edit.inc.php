  ...
  if (isset($_POST['do']) && $_POST['do'] == 'genPin') {
    if (!$config['csrf']['enabled'] || $config['csrf']['enabled'] && $csrftoken->valid) {
      if ($user->generatePin($_SESSION['USERDATA']['id'], $_POST['currentPassword'])) {
        $_SESSION['POPUP'][] = array('CONTENT' => 'Ваш PIN # был выслан на ваш email.', 'TYPE' => 'alert alert-success');
      } else {
      ...
      if (isset($_POST['unlock']) && isset($_POST['utype'])) {
        ...
        if ($isvalid) {
          ...
          if (!$config['csrf']['enabled'] || $config['csrf']['enabled'] && $csrftoken->valid) {
            ...
            if ($send) {
              $_SESSION['POPUP'][] = array('CONTENT' => 'Подтверждение было отправлено на ваш адрес электронной почты, следуйте этой ссылке, чтобы продолжить', 'TYPE' => 'alert alert-success');
            } else {
              $_SESSION['POPUP'][] = array('CONTENT' => $user->getError(), 'TYPE' => 'alert alert-danger');
            }
          } else {
        ...
        switch (@$_POST['do']) {
          case 'cashOut':
        	...
        	if ($setting->getValue('disable_payouts') == 1 || $setting->getValue('disable_manual_payouts') == 1) {
        	  $_SESSION['POPUP'][] = array('CONTENT' => 'Ручные выплаты отключены.', 'TYPE' => 'alert alert-warning');
          } else if ($config['twofactor']['enabled'] && $config['twofactor']['options']['withdraw'] && !$wf_editable) {
            $_SESSION['POPUP'][] = array('CONTENT' => 'Вы еще не разблокировали снятие средств.', 'TYPE' => 'alert alert-danger');
          } else if ($aBalance['confirmed'] < $config['mp_threshold']) {
            $_SESSION['POPUP'][] = array('CONTENT' => 'Баланс акаунта должно быть >= ' . $config['mp_threshold'] . ' совершить ручную выплату.', 'TYPE' => 'alert alert-warning');
          } else if (!$coin_address->getCoinAddress($_SESSION['USERDATA']['id'])) {
            $_SESSION['POPUP'][] = array('CONTENT' => 'У вас нет номера выплат.', 'TYPE' => 'alert alert-danger');
        	} else {
        	  $user->log->log("info", $_SESSION['USERDATA']['username']." запрос ручной выплаты");
        	    ...
        	    if (!$oPayout->isPayoutActive($_SESSION['USERDATA']['id'])) {
        	      if (!$config['csrf']['enabled'] || $config['csrf']['enabled'] && $csrftoken->valid) {
        	        if ($iPayoutId = $oPayout->createPayout($_SESSION['USERDATA']['id'], $oldtoken_wf)) {
        	          $_SESSION['POPUP'][] = array('CONTENT' => 'Создал новый запрос на выплату вручную с помощью ID #' . $iPayoutId);
        	        } else {
        	      ...
        	      $_SESSION['POPUP'][] = array('CONTENT' => 'У вас уже есть один активный запрос на выплату вручную.', 'TYPE' => 'alert alert-danger');
        	    }
        	  } else {
        	    $_SESSION['POPUP'][] = array('CONTENT' => 'Недостаточно средств, вам нужно больше, чем ' . $config['txfee_manual'] . ' ' . $config['currency'] . ' для покрытия транзакционных сборов', 'TYPE' => 'alert alert-danger');
        	  }
        	}
        	break;
          ...
          case 'updateAccount':
            if ($config['twofactor']['enabled'] && $config['twofactor']['options']['details'] && !$ea_editable) {
              $_SESSION['POPUP'][] = array('CONTENT' => 'Вы еще не разблокировали обновления учетной записи.', 'TYPE' => 'alert alert-danger');
              ...
              if ($user->updateAccount($_SESSION['USERDATA']['id'], $_POST['paymentAddress'], $_POST['payoutThreshold'], $_POST['donatePercent'], $_POST['email'], $_POST['timezone'], $_POST['is_anonymous'], $oldtoken_ea)) {
                $_SESSION['USERDATA']['timezone'] = $_POST['timezone'];
              	$_SESSION['POPUP'][] = array('CONTENT' => 'Обновлены данные учетной записи', 'TYPE' => 'alert alert-success');
              } else {
              	$_SESSION['POPUP'][] = array('CONTENT' => 'Не удалось обновить свою учетную запись: ' . $user->getError(), 'TYPE' => 'alert alert-danger');
              }
            } else {
              $_SESSION['POPUP'][] = array('CONTENT' => $csrftoken->getErrorWithDescriptionHTML(), 'TYPE' => 'alert alert-warning');
            }
