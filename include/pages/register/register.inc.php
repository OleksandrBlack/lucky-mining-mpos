...
if ($setting->getValue('disable_invitations') && $setting->getValue('lock_registration')) {
  ...
  $_SESSION['POPUP'][] = array('CONTENT' => 'Разрешено регистрировать только приглашенных пользователей.', 'TYPE' => 'alert alert-danger');
} else {
  ...
  if (!$config['csrf']['enabled'] || $config['csrf']['enabled'] && $csrftoken->valid) {
    if ($setting->getValue('recaptcha_enabled') != 1 || $setting->getValue('recaptcha_enabled_registrations') != 1 || $rsp->is_valid) {
      ...
      if ($config['check_valid_coinaddress'] AND empty($validcoinaddress)) {
        $_SESSION['POPUP'][] = array('CONTENT' => 'Введите действительный адрес кошелька.', 'TYPE' => 'alert alert-danger');
      } else {
        if ($user->register(@$_POST['username'], $validcoinaddress, @$_POST['password1'], @$_POST['password2'], @$_POST['pin'], @$_POST['email1'], @$_POST['email2'], @$_POST['tac'], $token)) {
          (!$setting->getValue('accounts_confirm_email_disabled')) ? $_SESSION['POPUP'][] = array('CONTENT' => 'Пожалуйста, проверьте свой почтовый ящик, чтобы активировать эту учетную запись.') : $_SESSION['POPUP'][] = array('CONTENT' => 'Account created, please login');
        } else {
